...
import {
  connectServer,
  disconnectServer,
  testConnection as testSshConnection,
} from "../services/ssh";
...
export const useServerStore = create<ServerStore>((set, get) => ({
  ...
  connectToServer: async (id) => {
    const others = get().servers.filter(
      (s) => s.status === "connected" && s.id !== id
    );
    for (const server of others) {
      await get().disconnectServer(server.id);
    }
    set({ activeServerId: id });
  ...
  updateServer: async (id, values) => {
    const previousStatus = get().servers.find(
      (server) => server.id === id
    )?.status;
      ...
      servers: state.servers.map((item) =>
        item.id === id
          ? {
              ...server,
              status:
                previousStatus === "connected" ? "disconnected" : item.status,
            }
          : item
      ),
    }));
